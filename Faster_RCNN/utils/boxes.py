...
# -------------------------------------------------------------------------------
# Name:         boxes.py
# Description:  对boxes的一些操作，包括：IoU，nms，将越界的坐标调整到图片边界上，移除宽高小于指定阈值的boxes
# Description:  主要使用在RPN网络中对proposal进行过滤、正样本和负样本的确定
# Author:       WaBiKong
# Date:         2021/11/18
# -------------------------------------------------------------------------------
...
def remove_small_boxes(boxes, min_size):
    ...
    # torch.logical_and: 逻辑与，torch1.5版本以上才适用
    keep = []
    for i in range(len(ws)):
        if ws[i] >= min_size and hs[i] >= min_size:
            keep.append(True)
        else:
            keep.append(False)
