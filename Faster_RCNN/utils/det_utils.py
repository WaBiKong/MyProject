...
# -------------------------------------------------------------------------------
# Name:         det_utils.py
# Description:  平衡正负样本类BalancedPositiveNegativeSampler
# Description:             用于RPN计算损失时选取适当数量的随机正负样本(anchors)和选取适当数量的随机正负样本(proposals)进入ROI
# Description:  BoxCoder类，包含操作encode(根据gtbox和生成的anchors形成真实回归参数)
# Description:                  和decod(将预测的回归参数应用到生成的anchors形成proposal)
# Description:  Matcher类: 匹配生成的anchors与gt boxes或匹配proposals与gt boxex
# Author:       WaBiKong
# Date:         2021/11/18
# -------------------------------------------------------------------------------
...
# 平衡正负样本 num_pos + num_neg = batch_size_per_image
...
class BoxCoder(object):
    ...
    def decode(self, rel_codes, boxes):
        ...
        # 将一个批量大小的图片的anchors全部拼接，形成和预测的anchors回归参数一样的格式
        concat_boxes = torch.cat(boxes, dim=0)

        # 计算一个batch中anchors的总数
        box_sum = 0
    ...
    def decode_single(self, rel_codes, boxes):
        ...
        """
        boxes = boxes.to(rel_codes.dtype)  # 将生成的anchors设置成预测的anchors回归参数一样的类型
        ...
        dx = rel_codes[:, 0::4] / wx   # 预测anchors/proposals的中心坐标x回归参数    0: :4 可以保证取到的dx有两个维度 [anchors, 1]
        ...
        dw = torch.clamp(dw, max=self.bbox_xform_clip)  # clamp限制数值上下限
...
# 将生成的anchors与gt boxes进行匹配，并确保所有的gt boxes都匹配给了anchors
class Matcher(object):
    ...
    # 确保每个gt box都有anchors和之匹配，即不漏掉任何一个gt box
