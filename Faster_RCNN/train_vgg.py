...
def main():
    ...
    if not os.path.exists("save_weights/vgg"):
        os.makedirs("save_weights/vgg")
    # 图像预处理
    data_transform = {
    ...
    model = create_model(backbone_name='vgg', num_classes=21)
    # print(model)
    ...
    torch.save(model.state_dict(), "./save_weights/vgg/pretrain.pth")
    ...
    for epoch in range(init_epochs, num_epochs + init_epochs, 1):
        ...
        if epoch in range(num_epochs + init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/vgg/vgg-model-{}.pth".format(epoch))
