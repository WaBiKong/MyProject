...
from myDataset import transforms
...
def main():
    ...
    if not os.path.exists("save_weights/mobilenetv2"):
        os.makedirs("save_weights/mobilenetv2")
    # 图像预处理
    data_transform = {
    ...
    train_dataset = VOC.VOC2012Dataset(VOC_root, data_transform["train"], train_set=True)
    train_sampler = None
    ...
    val_dataset = VOC.VOC2012Dataset(VOC_root, data_transform["val"], train_set=False)
    ...
    model = create_model(backbone_name='MobileNetV2', num_classes=21)
    # print(model)
    ...
    for epoch in range(init_epochs, num_epochs + init_epochs, 1):
        ...
        if epoch in range(num_epochs + init_epochs)[-5:]:
            save_files = {
                ...
                'epoch': epoch}
            torch.save(save_files, "./save_weights/mobilenetv2/mobile-model-{}.pth".format(epoch))
